...
%AAAs are characterized by localized aortic dilations, altering hemodynamics and increasing rupture risk [\cite{Lattanzi2020}]. Assessing this risk is complex, relying on morphology, patient-specific factors like age, medical history, and aneurysm progression. Fluid-dynamic parameters show promise as rupture predictors [\cite{Bologna2023}], nonetheless, the reliability of CFD models of blood flow heavily depends on boundary conditions that are typically unknown or affected by uncertainty. On the other hand flow-encoded time-resolved phase contrast magnetic resonance imaging (4DFlow MRI) can be adopted to assess blood flow dynamics in AAAs non-invasively [\cite{Dyverfeldt2015}]. However, velocity measurements obtained with this imaging modality are noisy and typically violate the mass conservation law and are characterized by low spatiotemporal resolution. VarDA offers a rigorous framework to incorporate velocity observations obtained through 4D flow MRI within CFD simulations of blood flow, effectively reducing the limitations and enhancing the potential of the two approaches.
...
%Several studies have examined VarDA's application with a Coupled solver in hemodynamics. Elia et al. [\cite{Delia2013}] compared a splitting method, sensitive to noise, with a control-based approach in 2D Stokes flow simulations. While more robust by optimizing inflow normal stress to match velocity fields, the proposed approach suffered from high computational cost, making it not directly applicable to real 3D case scenarios. Further research applied VarDA to Navier-Stokes equations, using a Discrete Optimization strategy to incorporate noisy velocity measurements [\cite{Delia2012}]. Initial tests on a 2D aortic arch and later studies on a carotid bifurcation confirmed VarDA's effectiveness in reconstructing flow from synthetic data. Advanced applications, including a 3D saccular aneurysm, showcased the method's capacity to manage various boundary controls and optimization techniques [\cite{Tiago2017}]. Koltukluo\u{g}lu's work [\cite{Koltuklouglu2018}] demonstrated VarDA's superiority in maintaining flow congruence across domains with 4DFlow MRI compared to traditional CFD, especially near inlets. Funke's application [\cite{Funke2019}] of 4DVarDA to transient blood flow analysis in aneurysms, utilizing phase contrast-MRI data and highlighting VarDA's capability to capture complex hemodynamic behaviors. Additionally, Dokken's multimesh finite element method [\cite{Dokken2020}] offered a robust framework for simulating flows in complex vascular geometries with IPCS, improving numerical stability and accuracy.
...
%The FE platform \emph{FEniCS} [\cite{alnaes2015fenics}] was chosen to solve the weak and discretized forms of the NS equations. The fluid domain $\Omega$ was discretized into triangular elements for 2D and tetrahedrons for 3D, using linear and quadratic shape functions for nodal pressure and velocity, respectively. A computational grid with a characteristic size of 1-1.5 mm was generated via a Python script developed in house. The explicit Crank-Nicolson (CN) time-integration scheme was applied, with a time increment $\Delta t = 0.001$ s. To handle the non-linearity of the Navier-Stokes equations and the pressure-velocity coupling, an IPCS [\cite{Dokken2020}] was implemented. This scheme consists of three main steps: 
%\begin{itemize} 
%  \item \textbf{Tentative velocity step}: solves the reaction-diffusion-convection equation to obtain an intermediate velocity. 
%  \item \textbf{Pressure-correction step}: solves a Poisson equation to adjust the pressure field. 
%  \item \textbf{Velocity update step}: corrects the velocity field using the updated pressure values. 
%\end{itemize} 
%A generalized minimal residual method (GMRES) was chosen as the linear solver, with tolerances of $1 \times 10^{-4}$ for both momentum and continuity equations.
